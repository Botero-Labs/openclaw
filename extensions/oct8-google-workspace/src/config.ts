...
function parseCredentialsConfig(raw: unknown, issues: string[]): CredentialsConfig | undefined {
  ...
  if (!clientId) {
    issues.push("credentials.clientId: required.");
  }
  if (!clientSecret) {
    issues.push("credentials.clientSecret: required.");
  }
  if (!refreshToken) {
    issues.push("credentials.refreshToken: required.");
  }
...
export function computeScopesForEmail(config: PluginConfig, email: string): string[] {
  ...
  return Array.from(scopeSet).toSorted();
}
