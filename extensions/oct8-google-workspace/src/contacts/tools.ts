...
export function registerContactsTools(
  ...
  if (contactsServices.length === 0) return;
  ...
  api.registerTool((ctx) => ({
    ...
    async execute(
      ...
      if (!resolved) return errorResult("No contacts service configured for this agent.");
  ...
  api.registerTool((ctx) => ({
    ...
    async execute(
      ...
      try {
        ...
        if (params.email) person.emailAddresses = [{ value: params.email, type: params.emailType }];
        if (params.phone) person.phoneNumbers = [{ value: params.phone, type: params.phoneType }];
