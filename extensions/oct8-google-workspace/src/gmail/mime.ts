...
export function buildMimeMessage(params: MimeParams): string {
  ...
  if (params.from) headers.push(`From: ${sanitizeHeader(params.from)}`);
  headers.push(`To: ${sanitizeHeader(params.to)}`);
  if (params.cc) headers.push(`Cc: ${sanitizeHeader(params.cc)}`);
  if (params.bcc) headers.push(`Bcc: ${sanitizeHeader(params.bcc)}`);
