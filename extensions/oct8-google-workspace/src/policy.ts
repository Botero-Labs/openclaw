...
export function checkPolicy(params: CheckPolicyParams): PolicyDecision {
  ...
  for (const [id, entry] of Object.entries(config.services)) {
    if (entry.service !== serviceKind || entry.email !== normalizedEmail) {
      continue;
    }
...
export function listAllowedActions(params: {
  ...
  for (const e of Object.values(config.services)) {
    if (e.service !== serviceKind || e.email !== normalizedEmail) {
      continue;
    }
  ...
  return allActions.filter((action) => {
    // Mode gate
    if (entry.mode === "delegated_human" && destructive.includes(action)) {
      return false;
    }
    // Allowlist gate
    if (entry.allowedActions !== undefined && !entry.allowedActions.includes(action)) {
      return false;
    }
    return true;
  });
